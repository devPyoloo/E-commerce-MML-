...
const RenderProducts = React.forwardRef(function RenderProducts({
  ...
  return (
    <div className="flex flex-col md:w-full">
      <main className="grid grid-cols-2 gap-x-5 gap-y-10 md:gap-x-16 md:grid-cols-4 md:gap-y-32 cursor-pointer">
        {sortedProducts.length > 0 ? (
          sortedProducts.map((product) => (
            <figure className="product-card" key={product.id}>
              <Link to={product.id.toString()}>
                <div className="bg-mutedgray mb-3 flex justify-center items-center 2xl:w-72 2xl:h-72 md:w-56 md:h-56 h-40 overflow-hidden">
